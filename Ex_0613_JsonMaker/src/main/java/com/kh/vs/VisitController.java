...
public class VisitController {
	...
	public String update(VisitVO vo) {
		...
		if(!photo.isEmpty()) {
			...
			//파일을 저장할 경로 생성 
			File saveFile = new File(savePath, filename); //절대 경로
			
			if(!saveFile.exists()) {
				saveFile.mkdirs();
			}else {
				//동일 파일명이 존재하는 경우 업로드 시간을 추가하여 중복을 방지하자.
				long time = System.currentTimeMillis(); // 1970년도부터 지금까지 경과된 시간을 가져온다.
				filename = String.format("%d_%s", time, filename);
				saveFile = new File(savePath, filename); // 절대 경로에 지금 올리려는 파일을 함께 객체화
